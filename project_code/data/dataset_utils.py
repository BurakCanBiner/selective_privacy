...
class UTKFace(BaseDataset):
    ...
    def __init__(self, config):
        ...
        if config["train"] is True:
            ...
            label_csv = pd.read_csv(config["path"] + "UTK_label_val.csv")
            config["path"] += "/val"
        self.label_csv = label_csv.set_index("file")
        cols = [cols for cols in self.label_csv.columns]
        super(UTKFace, self).__init__(config)
        self.label_mapping = {}
        self.label_mapping["race"] = {0: 0,
                                      1: 1,
                                      2: 2,
                                      3: 3,
                                      4: 4}
        self.label_mapping["gender"] = {0: 0, 1: 1}
    def load_label(self, filepath, label_type):
