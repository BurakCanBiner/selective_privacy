...
#         print(self.pruner_model,flush=True)
...
#         print(f"shape of the input is {self.unpruned_z_in.shape}")
# #         print("----------------------")
...
#             blocks = model_list[0]
#             classifier = model_list[1]
#             new = blocks[self.split_layer:]
            
#             self.model = nn.Sequential(*nn.ModuleList(list(new) + list(classifier)))
#             print(self.model)
            
#             test = torch.ones(256,64,8,8)
#             out = self.model(test)
...
#         print(f"shape of the tensor before network forward {x.shape}")
        return self.model(x)
